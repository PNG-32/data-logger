...
namespace Bits {
	...
	struct DataBank {
		...
		struct PACKED Header {
			...
			constexpr static Header fromLocation(AVRX::EEPROM::Address const location) {
				return {
					true,
		...
		/// @brief Entry size.
		constexpr static uint16 const	ENTRY_SIZE		= sizeof(EntryType);
		/// @brief Header size.
		constexpr static uint16 const	HEADER_SIZE		= sizeof(Header);

		/// @brief Logical maximum amount of entries.
		constexpr static usize const	MAX_SIZE		= MAX;
		/// @brief Accessbible maximum amount of entries.
		constexpr static usize const	MAX_CAPACITY	= (MAX_SIZE < 32 ? MAX_SIZE : 32);
		/// @brief True maximum amount of entries.
		constexpr static usize const	MAX_ENTRIES		= (EEPROM_SIZE - HEADER_SIZE) / ENTRY_SIZE;
		...
		DataBank(uint16 const location = 0): headerLocation(location) {
			byte v;
			if (AVRX::EEPROM::readByte(location))
				AVRX::EEPROM::readBlock(location, header);
		}
		...
		void fetch(uint8 const index, EntryType& output) const {
			if (index < size())
				AVRX::EEPROM::readBlock((header.entryStart + index * ENTRY_SIZE), output);
		}
		...
		void set(uint8 const index, EntryType const& entry) {
			if (index < size())
				AVRX::EEPROM::writeBlock((header.entryStart + index * ENTRY_SIZE), entry);
		}
	...
	private:
		...
		void updateHeader() {
			AVRX::EEPROM::writeBlock(headerLocation, header);
		}
	};
}
